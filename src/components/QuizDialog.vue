    ...
    <v-card>
      ...
      <v-card-text>
        <div v-if="step === 1">
          ...
          <likert :labels="probabilityLabels" v-model="stockProbability"></likert>
        </div>
        <div v-else-if="step === 2">
          <!-- Confidence Rating Likert Scale -->
          <p>How confident are you in this assessment?</p>
          <likert :labels="confidenceLabels" v-model="confidenceRating"></likert>
        </div>
      </v-card-text>
      <v-card-actions>
        <v-btn v-if="step === 1" size="x-large" :disabled="!isStep1Valid" variant="outlined" color="primary"
          @click="nextStep">
          Next
        </v-btn>
        <v-btn v-if="step === 2" size="x-large" :disabled="!isStep2Valid" variant="outlined" color="primary"
          @click="closeDialog">
          Submit
        </v-btn>
      </v-card-actions>
...
@media (max-width: 900px) {
  .v-slider-track__tick-label {
    ...
    /* Allow text to wrap */
    text-align: center;
    /* Optional: center-align wrapped text */
